...
describe('skills_add / skills_remove directives', () => {
  ...
  test('runtime: agents() folds directives for the selected preset', () => {
    const config: PluginConfig = {
      preset: 'p1',
      agents: { oracle: { skills: ['a'] } },
      presets: {
        p1: { agents: { oracle: { skills_add: ['b'] } } },
        p2: { agents: { oracle: { skills_remove: ['a'] } } },
      },
    };
    ...
    expect(switched.skills).toEqual([]);
    expectNoDirectiveKeys(switched);
  });

  test('runtime: selected runtime preset replaces startup-preset directive', () => {
    writeUserConfig({
      preset: 'p1',
      presets: {
  ...
  test('runtime: empty skills_add in selected preset suppresses startup directive', () => {
    writeUserConfig({
      preset: 'p1',
      presets: {
        p1: { oracle: { skills_add: ['b'] } },
